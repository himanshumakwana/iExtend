...
fn overlapping_rects_merge_into_bounding_box() {
    ...
    assert_eq!(out.len(), 1);
    assert_eq!(out[0], DamageRect { x: 0, y: 0, w: 150, h: 150 });
}
...
fn single_rect_unchanged() {
    let r = DamageRect { x: 10, y: 20, w: 30, h: 40 };
...
fn touching_rects_merge() {
    ...
    let a = DamageRect { x: 0, y: 0, w: 50, h: 50 };
    let b = DamageRect { x: 50, y: 0, w: 50, h: 50 }; // right edge of a == left edge of b
...
fn three_way_chain_merges_all() {
    ...
    let a = DamageRect { x: 0, y: 0, w: 60, h: 60 };
    let b = DamageRect { x: 50, y: 0, w: 60, h: 60 };  // overlaps a
    let c = DamageRect { x: 100, y: 0, w: 60, h: 60 }; // overlaps b (and ab)
    let out = coalesce(vec![a, b, c]);
    assert_eq!(out.len(), 1);
    assert_eq!(out[0], DamageRect { x: 0, y: 0, w: 160, h: 60 });
}
